...
async def validate_bootstrap_results(
    ...
    result.add_detail(
        "total_world_items",
        sum(len(items) for items in world_building.values() if isinstance(items, dict)),
    )
...
async def _reconcile_state_with_profiles(
    ...
    try:
        ...
        placeholder_names = {
            "Antagonist", "SupportingChar1", "SupportingChar2", "SupportingChar3"
        }
        ...
        names_to_check = {name for name in names_to_check if name not in placeholder_names}
...
async def _validate_plot_outline(
    plot_outline: dict[str, Any], result: BootstrapValidationResult
) -> None:
    ...
    important_fields = ["genre", "setting"]
    ...
    for field in required_fields:
        if field not in plot_outline:
            ...
            result.add_error(
                f"Plot outline field '{field}' contains placeholder value: {plot_outline[field]}"
            )
    ...
    if summary and len(summary.strip()) < 50:
        result.add_warning(
            f"Plot summary seems too short ({len(summary)} chars). Consider expanding."
        )
    ...
    if missing_recommended:
        result.add_warning(
            f"Plot outline missing optional recommended fields: {', '.join(missing_recommended)}"
        )
...
async def _validate_character_profiles(
    ...
    if len(character_profiles) < 2:
        result.add_warning(
            f"Only {len(character_profiles)} character(s) found. Stories typically need at least 2-3 characters."
        )
    ...
    for name, profile in character_profiles.items():
        ...
        if len(description.strip()) < 20:
            result.add_warning(
                f"Character '{name}' has very short description ({len(description)} chars)"
            )
        ...
        if hasattr(profile, "updates"):
            ...
            role = profile.get(
                "role", profile.get("updates", {}).get("role", "unknown")
            )
        ...
        if not traits:
            ...
            result.add_warning(
                f"Character '{name}' has only {len(traits)} trait(s). Consider adding more."
            )
        ...
        for trait in traits:
            if _is_fill_in_placeholder(trait):
                result.add_warning(
                    f"Character '{name}' has placeholder trait: '{trait}'"
                )
        ...
        if state_tracker:
            ...
            if metadata:
                if metadata["type"] != "character":
                    result.add_error(
                        f"Type conflict for '{name}': StateTracker shows type '{metadata['type']}' but validating as character"
                    )
                ...
                if similar_name and similar_name != name:
                    result.add_warning(
                        f"Character '{name}' has description similar to '{similar_name}'"
                    )
            else:
                # Character not found in StateTracker - this could indicate it wasn't properly reserved
                result.add_warning(
                    f"Character '{name}' not found in StateTracker - may indicate incomplete reservation process"
                )

    # Validate story structure requirements
    if not protagonist_found:
        result.add_error(
            "No protagonist character found. Every story needs a protagonist."
        )

    if not antagonist_found:
        result.add_warning(
            "No antagonist character found. Most stories benefit from a clear antagonist."
        )
    ...
    result.add_detail(
        ...
        [
            (
                profile.updates.get("role", "unknown")
                if hasattr(profile, "updates")
                else profile.get(
                    "role", profile.get("updates", {}).get("role", "unknown")
                )
...
async def _validate_character_uniqueness(
    ...
    for name in duplicate_names:
        result.add_error(
            f"Duplicate character name found: '{name}' appears {name_counts[name]} times"
        )
    ...
    if state_tracker:
        ...
        for name in untracked_names:
            result.add_warning(
                f"Character '{name}' exists in profiles but not tracked in StateTracker"
            )
        ...
        for name in extra_tracked:
            result.add_warning(
                f"Character '{name}' tracked in StateTracker but not in profiles"
            )
        ...
        tracked_duplicates = [
            name for name, count in tracked_name_counts.items() if count > 1
        ]
        for name in tracked_duplicates:
            result.add_error(
                f"StateTracker duplicate character name found: '{name}' appears {tracked_name_counts[name]} times"
            )
...
async def _validate_world_building(
    ...
    for category, items in world_categories.items():
        ...
        for item_name, item in items.items():
            ...
            if _is_fill_in_placeholder(item_name_val):
                result.add_error(
                    f"World item has placeholder name: '{item_name_val}' in category '{category}'"
                )

            if not item_description or item_description.strip() == "":
                result.add_warning(
                    f"World item '{item_name_val}' in category '{category}' has empty description"
                )
            elif len(item_description.strip()) < 20:
                result.add_warning(
                    f"World item '{item_name_val}' in category '{category}' has very short description ({len(item_description)} chars)"
                )

            # Check for placeholder content in description
            if _is_fill_in_placeholder(item_description):
                result.add_warning(
                    f"World item '{item_name_val}' has placeholder description"
                )
            ...
            if state_tracker:
                ...
                if metadata:
                    if metadata["type"] != "world_item":
                        result.add_error(
                            f"Type conflict for '{item_name_val}': StateTracker shows type '{metadata['type']}' but validating as world_item"
                        )

                    # Check for description similarity conflicts
                    similar_name = await state_tracker.has_similar_description(
                        item_description, "world_item"
                    )
                    if similar_name and similar_name != item_name_val:
                        result.add_warning(
                            f"World item '{item_name_val}' has description similar to '{similar_name}'"
                        )
                else:
                    # World item not found in StateTracker - this could indicate it wasn't properly reserved
                    result.add_warning(
                        f"World item '{item_name_val}' not found in StateTracker - may indicate incomplete reservation process"
                    )
    ...
    if total_items < 5:
        result.add_warning(
            f"Only {total_items} world items found. Rich stories benefit from more detailed world-building."
        )
    ...
    missing_key = [
        ...
        if cat not in world_categories or not world_categories[cat]
    ]
    if missing_key:
        result.add_warning(
            f"Missing key world building categories: {', '.join(missing_key)}. Consider adding for richer storytelling."
        )
    ...
    if len(missing_optional) == len(
        optional_categories
    ):  # Only warn if ALL are missing
        result.add_warning(
            "No cultural/religious/governmental elements found. Consider adding some for world depth."
        )
...
async def _validate_world_uniqueness(
    ...
    for name in duplicate_names:
        categories_with_name = [
            cat for item_name, cat in category_mapping.items() if item_name == name
        ]
        result.add_error(
            f"Duplicate world item name found: '{name}' appears in categories {categories_with_name}"
        )
    ...
    if state_tracker:
        ...
        for name in untracked_names:
            result.add_warning(
                f"World item '{name}' exists in profiles but not tracked in StateTracker"
            )
        ...
        for name in extra_tracked:
            result.add_warning(
                f"World item '{name}' tracked in StateTracker but not in profiles"
            )
        ...
        for name in tracked_duplicates:
            result.add_error(
                f"StateTracker duplicate world item name found: '{name}' appears {tracked_name_counts[name]} times"
            )
...
async def _validate_consistency(
    ...
    protagonist_in_profiles = any(
        (
            profile.updates.get("role")
            if hasattr(profile, "updates")
            else profile.get("role", profile.get("updates", {}).get("role"))
        )
        == "protagonist"
        for profile in character_profiles.values()
    )
    ...
    if plot_protagonist and plot_protagonist not in character_profiles:
        result.add_error(
            f"Plot protagonist '{plot_protagonist}' not found in character profiles"
        )

    if plot_protagonist and not protagonist_in_profiles:
        result.add_error(
            "Plot has protagonist name but no character profile has 'protagonist' role"
        )
    ...
    if plot_setting and not has_locations:
        result.add_warning(
            "Plot mentions setting but no locations found in world building"
        )

    # Check genre-world consistency
    genre = plot_outline.get("genre", "").lower()
    world_categories = [
        k for k in world_building.keys() if k not in ["is_default", "source"]
    ]
    ...
    if "fantasy" in genre:
        ...
        if not any(cat in world_categories for cat in fantasy_categories):
            result.add_warning(
                "Fantasy genre specified but no fantasy-specific world elements found"
            )
    ...
    if "sci-fi" in genre or "science fiction" in genre:
        ...
        if not any(cat in world_categories for cat in scifi_categories):
            result.add_warning(
                "Sci-fi genre specified but no sci-fi-specific world elements found"
            )

    # Validate character count matches story complexity
    story_complexity = (
        len(world_categories) + len(str(plot_outline.get("summary", ""))) // 100
    )
    character_count = len(character_profiles)

    if story_complexity > 5 and character_count < 3:
        result.add_warning(
            f"Complex world ({story_complexity} complexity score) but only {character_count} characters. Consider adding more characters."
        )
