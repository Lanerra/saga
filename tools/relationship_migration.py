...
from core.relationship_validator import RelationshipConstraintValidator, ValidationResult
...
class RelationshipMigrationTool:
    ...
    async def audit_existing_relationships(self, limit: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for record in results:
                ...
                if is_valid:
                    ...
                    if correction_result.is_valid and correction_result.validated_relationship != relationship_type:
                        relationship_data["suggested_correction"] = correction_result.validated_relationship
                        relationship_data["correction_confidence"] = correction_result.confidence
    ...
    async def fix_correctable_relationships(
        ...
        logger.info(f"Processing {len(correctable)} correctable relationships (dry_run={dry_run})")
        ...
        for rel_data in correctable:
            ...
            if not dry_run:
                try:
                    ...
                    logger.error(
                        f"Failed to fix relationship {rel_id}: {e}",
                        exc_info=True
                    )
            else:
    ...
    async def delete_uncorrectable_relationships(
        ...
        logger.info(f"Processing {len(uncorrectable)} uncorrectable relationships (dry_run={dry_run})")
        ...
        for rel_data in uncorrectable:
            ...
            if not dry_run:
                try:
                    ...
                    await neo4j_manager.execute_write_query(delete_query, {"rel_id": rel_id})
                    
                    deletion_record["success"] = True
                    deletion_results["successful_deletions"] += 1
                    
                    logger.info(f"Deleted uncorrectable relationship: {relationship_desc}")
                    
                except Exception as e:
                    deletion_record["error"] = str(e)
                    deletion_results["failed_deletions"] += 1
                    logger.error(
                        f"Failed to delete relationship {rel_id}: {e}",
                        exc_info=True
                    )
            else:
                # Dry run - just log what would be done
    ...
    async def restore_from_backup(self, backup_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            restore_results["current_relationships_deleted"] = delete_result[0].get("deletions", 0) if delete_result else 0
            
            logger.info(f"Deleted {restore_results['current_relationships_deleted']} existing relationships")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate relationships to enforce constraints")
    ...
    parser.add_argument("--min-confidence", type=float, default=0.5, help="Minimum confidence for corrections")
    ...
    try:
        if args.audit_only:
            # Only run audit
            results = await migration_tool.audit_existing_relationships(limit=args.limit)
            print("\n=== AUDIT RESULTS ===")
