...
class StateTracker:
    ...
    async def reserve(self, name: str, entity_type: str, description: str) -> bool:
        ...
        async with self._lock:
            if name in self._entities:
                logger.debug("Entity name already reserved", name=name, existing_type=self._entities[name]["type"])
                return False
    ...
    async def has_similar_description(self, description: str, entity_type: str | None = None) -> str | None:
            ...
            for name, metadata in self._entities.items():
                ...
                if len(description_lower) > 20 and len(existing_desc) > 20:
                    ...
                    if desc_words and existing_words:
                        similarity = len(desc_words.intersection(existing_words)) / len(desc_words.union(existing_words))
                        
                        if similarity >= config.STATE_TRACKER_SIMILARITY_THRESHOLD:
                            logger.debug("Similar description found", 
                                        name=name, 
