...
class AdaptiveConstraintSystem:
    ...
    def __init__(self, schema_introspector):
        ...
        self.min_samples = 3
        self.max_type_combinations_ratio = (
            0.7  # Max ratio of actual vs possible combinations
        )
        ...
        try:
            ...
            for rel_type, type_patterns in rel_patterns.items():
                if len(type_patterns) >= self.min_samples:
                    constraint = self._generate_statistical_constraint(
                        rel_type, type_patterns
                    )
            ...
            logger.info(
                f"Learned {len(new_constraints)} relationship constraints from {len(patterns)} patterns"
            )

            # Log some examples for debugging
            for rel_type, constraint in list(new_constraints.items())[:5]:
                logger.debug(
                    f"Learned constraint for {rel_type}: "
    ...
    def _generate_statistical_constraint(
        ...
        try:
            ...
            base_confidence = 1.0 - min(
                combination_ratio, self.max_type_combinations_ratio
            )
            frequency_boost = min(
                total_frequency / 100, 0.3
            )  # Max 30% boost from frequency
    ...
    def validate_relationship(
        ...
        if rel_type not in self.constraints:
            ...
            return (
                True,
                0.6,
                f"No constraints learned yet for {rel_type} - allowing creative usage",
            )
        ...
        if subject_valid and object_valid:
            return (
                ...
                f"Matches learned pattern (samples: {constraint.sample_size})",
            )
        ...
        suggestion_text = (
            "; ".join(suggestions)
            if suggestions
            else "exploring new creative territory"
        )
        ...
        return (
            True,
            0.4,
            f"Creative usage: {rel_type} {suggestion_text} - allowing for narrative flexibility",
        )
    ...
    def get_valid_relationships_for_types(
        ...
        for rel_type, constraint in self.constraints.items():
            if (
                subject_type in constraint.subject_types
                and object_type in constraint.object_types
            ):
    ...
    def get_relationship_stats(self, rel_type: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "type_combinations": len(constraint.subject_types)
            * len(constraint.object_types),
        }
    ...
    async def refresh_constraints_if_needed(self, max_age_hours: int = 12):
        ...
        if age.total_seconds() > (max_age_hours * 3600):
            logger.info(
                f"Constraints are {age.total_seconds() / 3600:.1f} hours old, refreshing..."
            )
            await self.learn_constraints_from_data()
