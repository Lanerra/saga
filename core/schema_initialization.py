...
async def initialize_dynamic_schema_system(
    ...
    try:
        ...
        dynamic_schema_manager.configure(
            learning_enabled=enable_learning if enable_learning is not None else config.settings.DYNAMIC_SCHEMA_LEARNING_ENABLED,
        ...
        if hasattr(dynamic_schema_manager, 'type_inference'):
            dynamic_schema_manager.type_inference.confidence_threshold = config.settings.DYNAMIC_TYPE_INFERENCE_CONFIDENCE_THRESHOLD
            dynamic_schema_manager.type_inference.min_pattern_frequency = config.settings.DYNAMIC_TYPE_PATTERN_MIN_FREQUENCY
        
        if hasattr(dynamic_schema_manager, 'constraint_system'):
            dynamic_schema_manager.constraint_system.min_confidence = config.settings.DYNAMIC_CONSTRAINT_CONFIDENCE_THRESHOLD
            dynamic_schema_manager.constraint_system.min_samples = config.settings.DYNAMIC_CONSTRAINT_MIN_SAMPLES
        
        if hasattr(dynamic_schema_manager, 'introspector'):
            dynamic_schema_manager.introspector.cache_ttl = config.settings.SCHEMA_INTROSPECTION_CACHE_TTL_MINUTES * 60
        ...
        try:
            ...
            if status.get("system", {}).get("initialized"):
                ...
                logger.info(f"Type patterns learned: {type_info.get('total_patterns', 0)}")
                logger.info(f"Relationship constraints learned: {constraint_info.get('total_constraints', 0)}")
            
            return True
            
        except asyncio.TimeoutError:
            logger.warning(f"Dynamic schema initialization timed out after {timeout_seconds}s - continuing with fallback")
            return False
    ...
    try:
        ...
        if has_error:
            ...
            if type_patterns == 0 and constraints == 0:
                ...
                message = "System initialized but no patterns learned (may be empty database)"
            else:
...
async def startup_dynamic_schema_integration(timeout_seconds: int = 30) -> bool:
    ...
    try:
        ...
        success = await initialize_dynamic_schema_system(timeout_seconds=timeout_seconds)
        
        if success:
            logger.info("Dynamic schema integration completed successfully")
        else:
            logger.warning("Dynamic schema initialization failed - falling back to static methods")
            
        return True  # Always return True - fallback is acceptable
        
    except Exception as e:
        logger.error(f"Critical error in dynamic schema startup integration: {e}", exc_info=True)
        return False
