...
from core.enhanced_node_taxonomy import ENHANCED_NODE_LABELS, NodeClassification
...
ENHANCED_RELATIONSHIP_CONSTRAINTS = {
    ...
    "LOVES": {
        ...
        "valid_object_types": NodeClassification.CONSCIOUS
        | {"Location", "Object", "Artifact", "Concept"},
    ...
    "FEARS": {
        ...
        - {"Entity"},  # Can fear almost anything
        "invalid_combinations": [],
        "bidirectional": False,
    ...
    "PRODUCES": {
        "valid_subject_types": NodeClassification.CONSCIOUS
        | {"Settlement", "Region", "Organization"},
    ...
    "CONSUMES": {
        "valid_subject_types": NodeClassification.CONSCIOUS
        | {"Settlement", "Organization", "System"},
...
ENHANCED_SEMANTIC_RULES = {
    "divine_interaction": {
        ...
        "validator": lambda s, o: (s in NodeClassification.CONSCIOUS)
        or (o in {"Deity", "Spirit"} and s in {"Artifact", "Symbol", "Structure"}),
    },
    "crafting_logic": {
        "relationships": ["CRAFTED_BY", "FORGED_FROM", "MADE_OF"],
        "rule": "Only physical objects can be crafted or made from materials",
        "validator": lambda s, o: s in {"Object", "Artifact", "Item", "Structure"}
        and (o in {"Material", "Resource"} or o in NodeClassification.CONSCIOUS),
    },
    "communication_requirements": {
        "relationships": ["SPEAKS", "TEACHES", "TELLS_OF"],
        "rule": "Communication requires conscious entities or information systems",
        "validator": lambda s, o: (
            s in NodeClassification.CONSCIOUS
            or s in {"System", "Document", "Organization"}
        ),
    },
    "temporal_consistency": {
        ...
        "validator": lambda s, o: (
            s in NodeClassification.TEMPORAL and o in NodeClassification.TEMPORAL
        ),
    },
    "spatial_logic": {
        ...
        "validator": lambda s, o: (
            s in NodeClassification.PHYSICAL_PRESENCE
            and o in NodeClassification.PHYSICAL_PRESENCE
        ),
    },
    "magical_interaction": {
        "relationships": ["ENCHANTED_BY", "POWERED_BY", "CURSED_BY"],
        "rule": "Magical interactions require magic systems or supernatural entities",
        "validator": lambda s, o: o in {"Magic", "Deity", "Spirit", "Person"}
        or s in {"Magic", "System"},
    },
}


def get_enhanced_relationship_suggestions(
    ...
    for rel_type, constraints in ENHANCED_RELATIONSHIP_CONSTRAINTS.items():
        if (
            subject_type in constraints["valid_subject_types"]
            and object_type in constraints["valid_object_types"]
        ):
            # Check for invalid combinations
            if (subject_type, object_type) not in constraints.get(
                "invalid_combinations", []
            ):
                ...
                suggestions.append(
                    (rel_type, constraints["description"], min(confidence, 1.0))
                )
...
def analyze_constraint_improvement() -> dict[str, Any]:
    ...
    enhanced_relationships = sum(
        1
        for rel in ENHANCED_RELATIONSHIP_CONSTRAINTS.keys()
    ...
    return {
        ...
        "improvement_metrics": {
            ...
            "type_utilization": len(ENHANCED_NODE_LABELS)
            / 15
            * 100,  # vs original 15 types
        },
    }
