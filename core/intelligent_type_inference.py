...
class IntelligentTypeInference:
    ...
    async def learn_from_existing_data(self, sample_size: int = 5000):
        ...
        async with self._learning_lock:
            try:
                logger.info(f"Learning type patterns from up to {sample_size} existing entities...")
                
                # Get sample data from introspector
                samples_by_label = await self.introspector.sample_node_properties(sample_size)
                ...
                pattern_count = sum(len(counters) for counters in self.learned_patterns.values())
                logger.info(f"Learned {pattern_count} patterns from {total_learned} entities across {len(samples_by_label)} labels")
                
            except Exception as e:
                logger.error(f"Failed to learn from existing data: {e}", exc_info=True)
    
    def _extract_patterns(self, name: str, category: str, description: str, type_property: str, actual_label: str):
        ...
        for word in words:
            if len(word) >= 3:  # Focus on meaningful words
                self.learned_patterns[f"word:{word}"][actual_label] += 2  # Higher weight
        ...
        if category and category.strip():
            ...
            self.learned_patterns[f"category:{clean_category}"][actual_label] += 3  # High weight
        ...
        if description and description.strip():
            desc_words = re.findall(r'\b\w{4,}\b', description.lower())[:5]  # First 5 meaningful words
            for word in desc_words:
                if word not in ['this', 'that', 'with', 'from', 'they', 'have', 'been', 'were', 'will']:
    ...
    def _cleanup_patterns(self):
        ...
        for pattern_key, label_counts in self.learned_patterns.items():
            ...
            for label, count in label_counts.items():
                if count < 2 and total_count > 10:  # Remove outliers in high-frequency patterns
    ...
    def infer_type(self, name: str, category: str = "", description: str = "") -> Tuple[str, float]:
        """Infer node type with confidence score using learned patterns."""
        if not name or not name.strip():
            return "Entity", 0.0
        
        if not self.learned_patterns:
            logger.debug("No learned patterns available, learning disabled or not initialized")
            return "Entity", 0.0
        ...
        confidence = min(best_score / max(total_weight, 1.0), 1.0) if total_weight > 0 else 0.0
        
        # Apply confidence smoothing to avoid overconfidence
        confidence = confidence * 0.9 if confidence > 0.8 else confidence
        
        logger.debug(f"Inferred type '{best_type}' for '{name}' with confidence {confidence:.3f}")
        return best_type, confidence
    
    def _add_pattern_scores(self, pattern_key: str, scores: Dict[str, float], weight: float):
    ...
    def _get_most_common_patterns(self, limit: int) -> List[Dict[str, any]]:
        ...
        for pattern_key, label_counts in self.learned_patterns.items():
            total_count = sum(label_counts.values())
            most_common_label = label_counts.most_common(1)[0] if label_counts else ("Unknown", 0)
        ...
        return sorted(pattern_frequencies, key=lambda x: x["total_frequency"], reverse=True)[:limit]
    ...
    async def refresh_patterns_if_needed(self, max_age_hours: int = 24):
        ...
        if age.total_seconds() > (max_age_hours * 3600):
            logger.info(f"Patterns are {age.total_seconds() / 3600:.1f} hours old, refreshing...")
            await self.learn_from_existing_data()