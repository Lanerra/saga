...
def register_database_services():
    ...
    # Register the Neo4j database service as a singleton
    register_singleton(
        name="database_service",
        factory=DatabaseServiceFactory.create_neo4j_service,
        dependencies=[],
        interface=DatabaseInterface
    )
    
    # Register factory for creating additional database connections if needed
    register_singleton(
        name="database_factory",
        factory=lambda: DatabaseServiceFactory(),
        dependencies=[],
        interface=None
    )
    
    logger.info("Database services registered with service registry")


def get_database_service() -> DatabaseInterface:
    """Get the database service from the registry."""
    from core.service_registry import resolve
    return resolve("database_service")


# Migration helper functions for gradual transition from singleton
async def migrate_from_singleton(query_func, *args, **kwargs):
    """
    Helper function to gradually migrate from singleton usage to service-based usage.
    
    This allows existing code to work while we transition to the new DI pattern.
    """
    try:
        # Try to get from service registry first
        db_service = get_database_service()
        method_name = query_func.__name__
        
        if hasattr(db_service, method_name):
            method = getattr(db_service, method_name)
            return await method(*args, **kwargs)
        else:
            # Fallback to original singleton method
            return await query_func(*args, **kwargs)
    except Exception:
        # If service not available, fall back to singleton
        return await query_func(*args, **kwargs)