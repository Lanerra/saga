...
class TestSyncFullState:
    """Tests for syncing full character state."""

    async def test_sync_full_state_empty(self, monkeypatch):
        """Test syncing empty character profiles."""
        ...
        result = await character_queries.sync_full_state_from_object_to_db({})
        assert result is True

    async def test_sync_full_state_single_character(self, monkeypatch):
        """Test syncing single character profile."""
        mock_execute = AsyncMock(return_value=None)
        monkeypatch.setattr(
            character_queries.neo4j_manager, "execute_cypher_batch", mock_execute
        )
        ...
        result = await character_queries.sync_full_state_from_object_to_db(profiles)
        assert result is True

    async def test_sync_full_state_with_relationships(self, monkeypatch):
        """Test syncing character with relationships."""
        mock_execute = AsyncMock(return_value=None)
        monkeypatch.setattr(
            character_queries.neo4j_manager, "execute_cypher_batch", mock_execute
        )
    ...
    async def test_sync_full_state_marks_deleted(self, monkeypatch):
        """Test syncing marks characters as deleted."""
        mock_execute = AsyncMock(return_value=None)
        monkeypatch.setattr(
            character_queries.neo4j_manager, "execute_cypher_batch", mock_execute
        )
        mock_read = AsyncMock(
            return_value=[{"name": "OldCharacter"}]
        )
        monkeypatch.setattr(
            character_queries.neo4j_manager, "execute_read_query", mock_read
        )
