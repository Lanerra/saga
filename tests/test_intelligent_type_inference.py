...
class TestPatternLearning:
    ...
    async def test_learn_from_existing_data_success(self):
        ...
        mock_introspector.sample_node_properties.return_value = {
            "Character": [
                {"name": "Alice", "category": "character", "description": "A brave warrior"},
                {"name": "Bob", "category": "character", "description": "A wise sage"},
            ],
            "Location": [
                {"name": "Castle", "category": "location", "description": "A fortress"},
                {"name": "Forest", "category": "location", "description": "A dark woods"},
            ]
        }
    ...
    async def test_learn_from_existing_data_exception_handling(self):
        ...
        mock_introspector = AsyncMock()
        mock_introspector.sample_node_properties.side_effect = Exception("Database error")
...
class TestPatternExtraction:
    ...
    def test_extract_patterns_short_name(self):
        ...
        category_patterns = [k for k in inference.learned_patterns.keys() if k.startswith("category:")]
        word_patterns = [k for k in inference.learned_patterns.keys() if k.startswith("word:")]
    ...
    def test_extract_patterns_description_filtering(self):
        ...
        desc_patterns = [k for k in inference.learned_patterns.keys() if k.startswith("desc_word:")]
        
        # Should include "brave" and "warrior"
        meaningful_words = ["desc_word:brave", "desc_word:warrior", "desc_word:fighting"]
        assert any(word in inference.learned_patterns for word in meaningful_words)
        
        # Should exclude common words
        common_words = ["desc_word:this", "desc_word:will", "desc_word:have", "desc_word:been"]
...
class TestPatternCleanup:
    ...
    def test_cleanup_patterns_low_frequency(self):
        ...
        inference.learned_patterns["low_freq_pattern"]["Character"] = 1  # Below threshold
        inference.learned_patterns["high_freq_pattern"]["Character"] = 5  # Above threshold
...
class TestTypeInference:
    ...
    def test_infer_type_with_description_match(self):
        ...
        result_type, confidence = inference.infer_type("Unknown", description="A mighty fortress")
...
def sample_learning_data():
    ...
    return {
        "Character": [
            {"name": "Alice", "category": "character", "description": "A brave warrior"},
            {"name": "Bob", "category": "character", "description": "A wise sage"},
            {"name": "Charlie", "category": "character", "description": "A cunning rogue"},
        ],
        "Location": [
            {"name": "Castle", "category": "location", "description": "A fortress"},
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_learning_and_inference_workflow(self, sample_learning_data):
        ...
        new_char_type, new_char_conf = inference.infer_type("David", category="character")
