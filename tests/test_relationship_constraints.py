...
class TestRelationshipSemantics:
    ...
    def test_invalid_spatial_relationships(self):
        ...
        is_valid, errors = validate_relationship_semantics(
            "Trait", "LOCATED_IN", "Location"
        )
        assert not is_valid
        assert any("Invalid subject type" in error for error in errors)

        is_valid, errors = validate_relationship_semantics(
            "Character", "LOCATED_IN", "Character"
        )
        assert not is_valid
    ...
    def test_social_relationship_constraints(self):
        ...
        is_valid, errors = validate_relationship_semantics(
            "Character", "FAMILY_OF", "WorldElement"
        )
        assert not is_valid
    ...
    def test_cognitive_relationship_constraints(self):
        ...
        is_valid, errors = validate_relationship_semantics(
            "Character", "BELIEVES", "Trait"
        )
        assert is_valid

        is_valid, errors = validate_relationship_semantics(
            "Character", "REMEMBERS", "Location"
        )
        assert is_valid

        # Invalid: non-conscious entities can't have cognitive relationships
        is_valid, errors = validate_relationship_semantics(
            "WorldElement", "BELIEVES", "Trait"
        )
        assert not is_valid

        is_valid, errors = validate_relationship_semantics(
            "Location", "REMEMBERS", "Character"
        )
        assert not is_valid
...
class TestValidationResult:
    ...
    def test_invalid_result_creation(self):
        ...
        result = ValidationResult(
            is_valid=False, original_relationship="MOUNTAIN_LOVES", errors=errors
        )
...
class TestIntegrationScenarios:
    ...
    def test_common_valid_relationships(self):
        ...
        for subject_type, predicate, object_type in test_cases:
            result = validate_relationship_constraint(
                subject_type, predicate, object_type
            )
            assert (
                result.is_valid
            ), f"Expected {subject_type} {predicate} {object_type} to be valid"
...
class TestConfigurationDrivenValidation:
    ...
    def test_auto_correct_disabled(self):
        ...
        try:
            ...
            assert (
                result.validated_relationship in ["RELATES_TO", "ASSOCIATED_WITH"]
