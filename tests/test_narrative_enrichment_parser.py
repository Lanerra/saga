...
async def test_extract_chapter_embeddings(sample_narrative_text, sample_chapter_data):
    ...
    with patch("core.parsers.narrative_enrichment_parser.get_chapter_data_from_db") as mock_get_chapter:
        mock_get_chapter.return_value = {"id": "chapter_1", "number": 1, "title": "Test Chapter", "summary": "Test summary", "act_number": 1, "embedding": None, "created_chapter": 1, "is_provisional": False}
        
        # Mock the _generate_embedding_vector method
        with patch.object(
            parser, "_generate_embedding_vector", new_callable=AsyncMock
...
async def test_parse_and_persist(sample_narrative_text, sample_character_data, sample_chapter_data):
    ...
    with patch.object(
        parser, "extract_physical_descriptions", new_callable=AsyncMock
    ) as mock_extract_physical:
        ...
        with patch.object(
            parser, "extract_chapter_embeddings", new_callable=AsyncMock
        ) as mock_extract_chapter:
            ...
            with patch.object(
                parser, "update_character_physical_descriptions", new_callable=AsyncMock
            ) as mock_update_physical:
                mock_update_physical.return_value = True
                
                # Mock the update_chapter_embeddings method
                with patch.object(
                    parser, "update_chapter_embeddings", new_callable=AsyncMock
                ) as mock_update_chapter:
