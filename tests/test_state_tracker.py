...
class TestStateTracker:
    ...
    async def test_reserve_new_entity(self, state_tracker, mock_config_enabled):
        ...
        result = await state_tracker.reserve(
            "TestCharacter", "character", "A brave protagonist"
        )
    ...
    async def test_reserve_duplicate_entity(self, state_tracker, mock_config_enabled):
        ...
        result = await state_tracker.reserve(
            "TestCharacter", "world_item", "A magic sword"
        )
    ...
    @pytest.mark.asyncio
    async def test_has_similar_description_basic(
        self, state_tracker, mock_config_enabled
    ):
        ...
        different_name = await state_tracker.has_similar_description(
            "A small furry creature that loves cheese", "character"
        )
        assert different_name is None

    @pytest.mark.asyncio
    async def test_has_similar_description_type_filter(
        self, state_tracker, mock_config_enabled
    ):
        """Test description similarity with type filtering."""
        # Reserve entities with similar descriptions but different types
        await state_tracker.reserve(
            "Character1", "character", "A brave warrior who fights in the great battles"
        )
        await state_tracker.reserve(
            "World1", "world_item", "A brave warrior who fights in the great battles"
        )

        # Test with character type filter
        similar_name = await state_tracker.has_similar_description(
            "A brave warrior who fights in the great battles", "character"
        )
        assert similar_name == "Character1"

        # Test with world_item type filter
        similar_name = await state_tracker.has_similar_description(
            "A brave warrior who fights in the great battles", "world_item"
        )
        assert similar_name == "World1"

    @pytest.mark.asyncio
    async def test_has_similar_description_disabled(
        self, state_tracker, mock_config_disabled
    ):
        """Test description similarity when StateTracker is disabled."""
        result = await state_tracker.has_similar_description(
            "Any description", "character"
        )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_duplicate_reservations(
        self, state_tracker, mock_config_enabled
    ):
    ...
    @pytest.mark.asyncio
    async def test_timestamp_parsing_edge_cases(
        self, state_tracker, mock_config_enabled
    ):
    ...
    @pytest.mark.asyncio
    async def test_description_similarity_edge_cases(
        self, state_tracker, mock_config_enabled
    ):
    ...
    async def test_state_tracker_with_character_validation(self):
        ...
        with patch("processing.state_tracker.config") as mock_config:
            ...
            await tracker.reserve(
                "Alice", "character", "A skilled archer from the northern lands"
            )
            await tracker.reserve(
                "Bob", "character", "A wise mage with ancient knowledge"
            )

            # Test conflict detection
            conflict_result = await tracker.reserve(
                "Alice", "world_item", "A magic bow"
            )
            ...
            similar_name = await tracker.has_similar_description(
                "A skilled archer from the northern lands", "character"
            )
    ...
    async def test_state_tracker_with_world_validation(self):
        ...
        with patch("processing.state_tracker.config") as mock_config:
            ...
            await tracker.reserve(
                "Mystic Forest",
                "world_item",
                "A dense forest filled with magical creatures",
            )
            await tracker.reserve(
                "Dragon Peak", "world_item", "A towering mountain where dragons nest"
            )
