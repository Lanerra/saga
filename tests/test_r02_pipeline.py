...
async def test_original_scene_punctuation_reaches_all_producers(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    scene = "Father O'Brien visits The Hague during The King's Return. King's Cross is nearby."
    catalog = select_catalog(selected_authority_state(
        tmp_path, ("Father O'Brien",), event_names=("The King's Return",),
        world_items=tuple(WorldItem(name=name, category="location", description="Named place", id=f"place-{index}")
                          for index, name in enumerate(["The Hague", "King's Cross"])),
    ))
    replies = iter([
    ...
    try:
        result = await extract_from_scene(scene, 0, 1, "Synthetic", "Fantasy", "Father O'Brien", "synthetic", catalog=catalog)
...
async def test_retained_scene_failure_publishes_no_partial_payload(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        assert len(requests) == 4
        errors = ["unnamed possessive descriptor", "ineligible", "ineligible", "unnamed possessive descriptor"]
        assert all(error in outcome["error"] for error, outcome in zip(errors, result["extraction_outcomes"], strict=True))
    finally:
        await service.aclose()
