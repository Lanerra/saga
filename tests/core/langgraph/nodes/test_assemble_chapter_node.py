...
class TestAssembleChapterEmptyScenes:
    ...
    def test_no_scene_drafts_ref(self, tmp_path: Path) -> None:
        state = {
...
class TestAssembleChapterSingleScene:
    ...
    def test_single_scene_content(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
    ...
    def test_single_scene_word_count(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
...
class TestAssembleChapterMultipleScenes:
    ...
    def test_two_scenes_joined_with_separator(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
        scenes = [
    ...
    def test_three_scenes_joined_with_separator(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
        scenes = [
    ...
    def test_multi_scene_word_count(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
        scenes = [
            "alpha beta gamma",
            "delta epsilon",
...
class TestAssembleChapterExternalization:
    ...
    def test_draft_and_scenes_externalized(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
    ...
    def test_draft_ref_metadata(self, tmp_path: Path) -> None:
        content_manager = ContentManager(str(tmp_path))
        scenes = ["Words here."]
