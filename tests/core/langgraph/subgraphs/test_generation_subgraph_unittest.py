...
class TestGenerationSubgraph(unittest.TestCase):
    def test_generation_subgraph_flow(self):
        async def run_test():
            # Setup mocks
            with patch("core.langgraph.nodes.scene_planning_node.llm_service") as mock_llm_plan, \
                 patch("core.langgraph.nodes.scene_generation_node.llm_service") as mock_llm_draft, \
                 patch("core.langgraph.nodes.context_retrieval_node.get_reliable_kg_facts_for_drafting_prompt", new_callable=AsyncMock) as mock_kg:
                
                # Mock planning response
                mock_llm_plan.async_call_llm = AsyncMock(return_value=(
                ...
                mock_llm_draft.async_call_llm = AsyncMock(side_effect=[
                    ("Draft for Scene 1", {}),
                    ("Draft for Scene 2", {})
                ])
                ...
                # Verify results
                self.assertEqual(result["draft_text"], "Draft for Scene 1\n\n# ***\n\nDraft for Scene 2")
