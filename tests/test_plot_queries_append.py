...
@pytest.mark.asyncio
async def test_append_plot_point_single_write_query_and_atomic_id(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    Append must:
    - use a single write query (no pre-read for max(sequence))
    - compute the next seq/id inside that write query
    """
    write_calls: list[tuple[str, dict[str, object]]] = []

    async def fake_write(query: str, params: dict[str, object] | None = None) -> list[dict[str, str]]:
